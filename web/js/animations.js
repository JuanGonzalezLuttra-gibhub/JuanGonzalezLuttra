...
/* ============================================================
   FAREWELL & CONTACT REVEAL — Animaciones cinematográficas por umbral
   - Despedida: animación línea por línea al scroll (Línea 1 -> 2 -> 3)
   - Contacto: secuencia escalonada (Stagger) de los 6 elementos de contacto
   ============================================================ */
function initFarewellAndContactReveal() {
  const reducedMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;

  const aboutSec = document.querySelector('.about');
  if (aboutSec) {
    if (reducedMotion) {
      aboutSec.classList.add('is-visible');
    } else {
      ...
      obsAbout.observe(aboutSec);
    }
  }

  const farewellSec = document.querySelector('.farewell');
  if (farewellSec) {
    if (reducedMotion) {
      farewellSec.classList.add('is-visible');
    } else {
      const obsFarewell = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
          if (entry.isIntersecting) {
            entry.target.classList.add('is-visible');
          } else {
      ...
      obsFarewell.observe(farewellSec);
    }
  }
  ...
  if (contactSec) {
    if (reducedMotion) {
      contactSec.classList.add('is-visible');
    } else {
      const obsContact = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
          if (entry.isIntersecting) {
            entry.target.classList.add('is-visible');
          } else {
